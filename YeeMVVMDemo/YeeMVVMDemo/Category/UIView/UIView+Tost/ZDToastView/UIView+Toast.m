...
- (void)showToast:(UIView *)toast position:(id)point {
    [self showToast:toast duration:CSToastDefaultDuration position:point];
}
...
- (CGSize)sizeForString:(NSString *)string font:(UIFont *)font constrainedToSize:(CGSize)constrainedSize lineBreakMode:(NSLineBreakMode)lineBreakMode {
    if ([string respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
        ...
        NSDictionary *attributes = @{NSFontAttributeName:font ?font : [UIFont systemFontOfSize:12], NSParagraphStyleAttributeName:paragraphStyle};
